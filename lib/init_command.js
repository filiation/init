...
const BUILT_IN = {
  toLCC(str) {
    let text = str;
    return text.replace(/-(\w)/g, (all, letter) => {
      return letter.toUpperCase();
    })
  },

  toUCC (str) {
    const text = BUILT_IN.toLCC(str)
...
module.exports = class Command {
  constructor() {
    this.name = 'hio-init';
    this.configName = '@alife/hio-init-config';
  ...
  * run(cwd, args) {
   ...
   *
   * @param {Object} mapping - boilerplate config mapping, `{ simple: { "name": "addon", "package": "hio-boilerplate-addon", "description": "Simple hio addon  boilerplate" } }`
  ...
  * askForVariable(targetDir, templateDir) {
    ...
    try {
      ...
      if (questions.name && !questions.name.default) {
        const type = questions.type && questions.type.default;
        const ext = type ? `(?:${type}-)?` : '';
        const reg = new RegExp(`^(?:hio-)?${ext}`);
        questions.name.default = path.basename(targetDir).replace(reg, '');
      }
      // get git url
      if (questions.gitUrl && !questions.gitUrl.default) {
        questions.gitUrl.default = git.getGitUrl(targetDir);
      }
      // get git author
      if (questions.author && !questions.author.default) {
        questions.author.default = user.getUserAccount();
      }
    } catch (err) {
    ...
    if (this.argv.silent) {
      ...
      askList = askList.filter(item => !item.silent);
      return Object.assign(result, yield inquirer.prompt(askList));
    }
  }
  ...
  * processFiles(targetDir, templateDir) {
  ...
  getParser() {
    return yargs
      .usage('init hio project from boilerplate.\nUsage: $0 [dir] --type=addon')
      ...
      default: {
        if (/^https?:/.test(key)) {
          ...
          if (fs.existsSync(path.join(home, '.cnpmrc')) || fs.existsSync(path.join(home, '.tnpmrc'))) {
            url = 'http://registry.npm.alibaba-inc.com';
          }
          url = url.replace(/\/$/, '');
          return url;
        }
  ...
  * getTemplateDir() {
    let templateDir;
    // when use `hio-init --template=PATH`
  ...
  /**
   * fetch boilerplate mapping from `hio-init-config`
   * @param {String} [pkgName] - config package name, default to `this.configName`
   * @return {Object} boilerplate config mapping, `{ addon: { "name": "addon", "package": "hio-boilerplate-addon", "description": "hio addon boilerplate" } }`
   */
  * fetchBoilerplateMapping(pkgName) {
  ...
  replaceTemplate(content, scope) {
    const exp = /(\\)?{{\s*(\w+)(?:\.(\w+)\(\))?\s*}}/g;
    return content.toString().replace(exp, (block, skip, key, func) => {
      if (skip) {
      ...
      if (scope.hasOwnProperty(key)) {
        name = scope[key];
        if (func && typeof BUILT_IN[func] === 'function') {
          name = BUILT_IN[func](name);
        }
      } else {
        name = block;
      }
  ...
  * downloadBoilerplate(pkgName) {
    ...
    const saveDir = path.join(os.tmpdir(), 'hio-init-boilerplate');
