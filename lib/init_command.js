...
module.exports = class Command {
  constructor() {
    this.name = 'fli-init';
    this.configName = 'fli-init-config';
  ...
  * run(cwd, args) {
   ...
   *
   * @param {Object} mapping - boilerplate config mapping, `{ simple: { "name": "addon", "package": "fli-boilerplate-mobx", "description": "Simple mobx addon  boilerplate" } }`
  ...
  * processFiles(targetDir, templateDir) {
  ...
  getParser() {
    return yargs
      .usage('init project from boilerplate.\nUsage: $0 [dir] --type=addon')
  ...
  * getTemplateDir() {
    let templateDir;
    // when use `fli-init --template=PATH`
  ...
  /**
   * fetch boilerplate mapping from `fli-init-config`
   * @param {String} [pkgName] - config package name, default to `this.configName`
   * @return {Object} boilerplate config mapping, `{ addon: { "name": "mobx", "package": "fli-boilerplate-mobx", "description": "fli mobx boilerplate" } }`
   */
  ...
  * downloadBoilerplate(pkgName) {
    ...
    const saveDir = path.join(os.tmpdir(), 'fli-init-boilerplate');
